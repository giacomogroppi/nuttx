...
int bcm2711_mbox_getclkrate(uint8_t id, uint32_t *rate, bool measured)
{
  ...
  /* Returned clock rate is 0 when the clock does not exist (unless measured).
   * If the clock is not enabled, the return value for unmeasured clock rate
   * is the rate the clock will have when enabled.
   */
